...
const HrPanelPage = () => {
  ...
  const getHrData = () => {
    try {
      const hrData = localStorage.getItem('hr');
  ...
  const { data, error, isLoading, refetch } = useQuery({
    queryKey: ['hr-get-pending-leaves'],
    queryFn: async () => {
      const response = await axios.get(
        `${API_URL}/hr/get-pending-leaves?employeeCode=${hr['Employee Code']}`,
      );
  ...
  const statusMutation = useMutation({
    mutationFn: async ({
      ...
      const response = await axios.put(
        `${API_URL}/hr/change-leave-application-status`,
        { leaveId, status },
      );
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row gap-2 justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          <a
            href={`${API_URL}/download-employee-leaves`}
            download
            className="bg-[#ed1c24] text-white text-center font-semibold p-2 md:py-2 md:px-4 rounded text-xs sm:text-base"
          >
            Download Employee Leaves
          </a>
          <a
            href={`${API_URL}/download-hr-manager-leaves`}
            download
            className="bg-[#ed1c24] text-white text-center font-semibold p-2 md:py-2 md:px-4 rounded text-xs sm:text-base"
          >
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {data.map((leave: any) => (
            ...
            >
              <div
                className="p-4 flex justify-between gap-2 items-center cursor-pointer"
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-gray-500 flex gap-2 flex-col sm:flex-row">
                    Dept : {leave.Department}
                    <span>Desg : {leave.Designation}</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs p-1">
                    <MapPin className="sm:w-4 sm:h-4 w-3 h-3 text-gray-500" />
                    <span>{leave['Work Location']}</span>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  {(leave.Status === 'Manager Approved' ||
                    leave.Status === 'Partner Approved') &&
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>Designation: {leave.Designation}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Building className="w-4 h-4 text-gray-500" />
                      <span>Department: {leave.Department}</span>
                    </div>
                    <div className="flex items-center gap-2">
