...
const ManagerPanelPage = () => {
  ...
  const { data, error, isLoading, refetch } = useQuery({
    queryKey: ['manager-get-pending-leaves'],
    queryFn: async () => {
      const response = await axios.get(
        `${API_URL}/manager/get-pending-leaves?employeeCode=${manager['Employee Code']}`,
      );
  ...
  const statusMutation = useMutation({
    mutationFn: async ({
      ...
      const response = await axios.put(
        `${API_URL}/manager/change-leave-application-status`,
        { leaveId, status },
      );
            ...
            >
              ...
              >
                <div>
                  ...
                  <div className="text-sm text-gray-500 flex gap-2 flex-col sm:flex-row">
